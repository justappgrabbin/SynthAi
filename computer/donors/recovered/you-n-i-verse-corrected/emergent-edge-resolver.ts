 ...
 * 
 * The 36 edge types are NOT fixed connections.
 * They are EMERGENT — derived from the interaction of two states.
 * 
 * Architecture:
 *   source state + target state + coordinates + phase trace
 *   → resolve compatibility
 *   → derive one of 36 edge types
 *   → execute that edge function
 *   → produce the next state
 * 
 * The 36 edge types correspond to the 36 Human Design channels,
 * but they are selected dynamically based on relational conditions.
 * 
 * Each edge type is a FUNCTION, not a label.
 * It transforms the source and target states when executed.
 */

import { MicroAddress, MacroAddress, EmergentAddress, encodeMicro, decodeMicro } from './synthia-bridge';

// ============================================================================
// EDGE TYPE DEFINITIONS (36 Channel Functions)
// ============================================================================

export type EdgeType = 
  | 'INSPIRATION'      // 1-8: Creative spark, initiation of expression
  | 'BEAT'             // 2-14: Rhythmic pulse, life force direction
  | 'MUTATION'         // 3-60: Genetic change, ordered chaos
  | 'LOGIC'            // 4-63: Pattern recognition, doubt resolution
  | 'RHYTHM'           // 5-15: Cyclic flow, extremes moderation
  | 'INTIMACY'         // 6-59: Emotional bonding, sexual chemistry
  | 'ALPHA'            // 7-31: Leadership, collective direction
  | 'CONCENTRATION'    // 9-52: Focused attention, stillness power
  | 'AWAKENING'        // 10-20: Self-awareness, present moment
  | 'CURIOSITY'        // 11-56: Idea seeking, stimulation drive
  | 'OPENNESS'         // 12-22: Emotional grace, social caution
  | 'PRODIGAL'         // 13-33: Secret keeping, privacy cycles
  | 'DEPTH'            // 16-48: Skill mastery, talent recognition
  | 'ACCEPTANCE'       // 17-62: Opinion formation, detail focus
  | 'JUDGEMENT'        // 18-58: Pattern correction, vitality discernment
  | 'SYNTHESIS'        // 19-49: Wanting/rejection, resource synthesis
  | 'MONEY'            // 21-45: Material control, gathering power
  | 'STRUGGLE'         // 28-38: Risk/perseverance, purpose finding
  | 'RECOGNITION'      // 29-30: Commitment/desire, experiential depth
  | 'PRESERVATION'     // 27-50: Nurturing/values, care provision
  | 'INITIATION'       // 25-51: Shock/love, spiritual awakening
  | 'POWER'            // 34-57: Sacral/intuition, raw power
  | 'TRANSIENCE'       // 35-36: Change/crisis, experiential cycles
  | 'COMMUNITY'        // 37-40: Friendship/alone, heart bonding
  | 'MATURATION'       // 42-53: Growth/beginnings, developmental cycles
  | 'TRANSFORMATION'   // 32-54: Ambition/drive, root pressure
  | 'EMOTING'          // 39-55: Provocation/spirit, emotional wave
  | 'AWARENESS'        // 24-61: Rationalization/mystery, mental awareness
  | 'STRUCTURING'      // 23-43: Insight/assimilation, mental architecture
  | 'SURRENDER'        // 26-44: Ego/alertness, transmission
  | 'DISCOVERY'        // 46-29: Determination/perseverance, body discovery
  | 'ABSTRACTION'      // 47-64: Realization/confusion, mental abstraction
  | 'DETACHMENT'       // 41-30: Contraction/desire, fantasy fuel
  | 'COMPLETION'       // 61-24: Truth/mystery, mental completion
  | 'DETAILS'          // 62-17: Impeccability/opinion, logical details
  | 'TRUTH'            // 63-4: Doubt/answers, mental truth
  | 'IMAGINATION'      // 64-47: Confusion/realization, mental imagination
  | 'NONE';            // No compatible edge type
...
export interface EdgeFunction {
  ...
  /**
   * Compatibility check: can these two states form this edge?
   */
  compatible: (source: NodeState, target: NodeState) => boolean;

  /**
   * Execute the edge function: transform source and target states.
   */
  execute: (source: NodeState, target: NodeState) => { source: NodeState; target: NodeState; emergent: EmergentState };
}
...
export interface NodeState {
  ...
  phase: number;
  coherence: number;
  dimension: number;  // 0-4 (which dimension stage)
}
...
export interface EmergentState {
  ...
  coherence: number;
  resonance: number;  // Hz
  tension: number;   // 0-1 (destructive interference)
  amplification: number; // 0-1 (constructive interference)
  trace: string;       // Semantic trace of the interaction
}
...
/**
 * Two nodes are compatible if their states satisfy relational conditions:
 * 1. Gate harmonic relationship (opposite or complementary)
 * 2. Phase alignment (within tolerance)
 * 3. Dimensional resonance (same or adjacent dimension)
 * 4. Coherence threshold (both above minimum)
 * 5. Macro context alignment (same planet family or zodiac element)
 */

function checkCompatibility(source: NodeState, target: NodeState): {
  compatible: boolean;
  score: number;
  reasons: string[];
} {
  ...
  // Rule 1: Gate harmonic relationship
  const harmonicGate = ((source.micro.gate + 32) % 64);  // Opposite gate
  ...
  // Rule 5: Macro context alignment
  ...
  const compatible = score >= 0.5;  // Threshold for edge formation
...
export const EDGE_FUNCTIONS: EdgeFunction[] = [
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        resonance: 7.83 * 2,  // Double Schumann
        tension: 0,
  ...
  },
  {
    ...
    execute: (s, t) => {
      const mutation = Math.random() < 0.5 ? s : t;  // Random selection
      const emergent: EmergentState = {
        type: 'MUTATION',
  ...
  },
  {
    ...
    execute: (s, t) => {
      const force = s.amplitude * s.amplitude;  // Square for power
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        trace: `Awareness: Gates ${s.micro.gate + 1} rational ${rational.toFixed(3)} × Gate ${t.micro.gate + 1} mystery ${mystery.toFixed(3)} = ${aware.toFixed(3)}`
      };
      return {
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        trace: `Surrender: Gates ${s.micro.gate + 1} ego ${ego.toFixed(3)} × Gate ${t.micro.gate + 1} alert ${alert.toFixed(3)} = transmit ${transmit.toFixed(3)}`
      };
      return {
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        trace: `Discovery: Gates ${s.micro.gate + 1} determines ${determine.toFixed(3)} × Gate ${t.micro.gate + 1} perseveres ${persevere.toFixed(3)} = ${discover.toFixed(3)}`
      };
      return {
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        trace: `Detachment: Gates ${s.micro.gate + 1} contracts ${contract.toFixed(3)} × Gate ${t.micro.gate + 1} desires ${desire.toFixed(3)} = fantasy ${fantasy.toFixed(3)}`
      };
      return {
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        trace: `Details: Gates ${s.micro.gate + 1} impeccable ${impeccable.toFixed(3)} × Gate ${t.micro.gate + 1} opinion ${opinion.toFixed(3)} = ${detail.toFixed(3)}`
      };
      return {
  ...
  },
  {
    ...
    execute: (s, t) => {
      ...
      const emergent: EmergentState = {
        ...
        tension: confuse,
        amplification: realize,
        trace: `Imagination: Gates ${s.micro.gate + 1} confuses ${confuse.toFixed(3)} × Gate ${t.micro.gate + 1} realizes ${realize.toFixed(3)} = ${imagine.toFixed(3)}`
      };
      return {
...
export class EmergentEdgeResolver {
   ...
   */
  resolve(source: NodeState, target: NodeState): {
    compatible: boolean;
    score: number;
    edgeType: EdgeType;
    edgeName: string;
    ...
    // Step 2: Find matching edge functions
    const matches = EDGE_FUNCTIONS.filter(fn => fn.compatible(source, target));

    if (matches.length === 0) {
      // Generic compatibility but no specific channel match
      return {
        compatible: true,
        score: compatibility.score,
        edgeType: 'NONE',
    ...
    // Step 3: Select best match (first match for now, could be weighted)
    const bestMatch = matches[0];

    // Step 4: Execute edge function
   ...
   */
  resolveAll(source: NodeState, targets: NodeState[]): {
    source: NodeState;
    edges: ReturnType<typeof this.resolve>[];
    bestEdge: ReturnType<typeof this.resolve> | null;
  } {
    const edges = targets.map(t => this.resolve(source, t));
    const validEdges = edges.filter(e => e.compatible);

    const bestEdge = validEdges.length > 0
      ? validEdges.reduce((best, current) => current.score > best.score ? current : best)
      : null;
