/**
 * GENERATED FILE — esbuild type-erasure port of emergent-edge-resolver.ts (donor you-n-i-verse-corrected).
 * Generated: 2026-09-22, esbuild 0.x `--format=esm --loader=ts`; NO logic changes.
 * Import specifiers rewritten: './synthia-bridge' -> './synthia-bridge.ported.mjs' (Node has no extensionless TS resolution).
 * Canonical source: emergent-edge-resolver.ts (this directory). Regenerate if donor changes.
 */
function createChannel(name, sourceGate, targetGate, meaning, transform) {
  return (input) => {
...
var CHANNELS = {
  ...
  "ch_3_60": createChannel(
    ...
    (input) => ({
      ...
      phase: 0,
      // Reset phase - mutation creates new beginning
  ...
  "ch_7_31": createChannel(
    ...
    (input) => ({
      ...
      // Target dominates (leadership flows outward)
      phase: input.targetState.phase,
  ...
  "ch_9_52": createChannel(
    ...
    (input) => ({
      amp: input.sourceState.amplitude * 0.8,
      // Reduce noise
      phase: 0,
      // Stillness = no phase oscillation
  ...
  "ch_25_51": createChannel(
    ...
    (input) => ({
      ...
      phase: 0,
      // Shock resets phase
      coh: input.sourceState.coherence * 0.8
      // Shock reduces coherence temporarily
    })
  ),
  // ========================================================================
...
// EDGE RESOLVER
// ============================================================================
var EmergentEdgeResolver = class {
  channels;
  edgeHistory;
  constructor() {
  ...
  resolve(source, target) {
    const sourceGate = source.micro.gate + 1;
