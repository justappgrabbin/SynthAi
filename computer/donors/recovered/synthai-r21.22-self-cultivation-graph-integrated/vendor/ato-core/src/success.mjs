...
export class SuccessLedger {
    ...
    observe(personId, indicatorId, value, { context = null, address = null, source = 'person' } = {}) {
        ...
        if (!person)
            throw new SuccessError('PURPOSE_NOT_FOUND', personId);
        const indicator = person.purpose.indicators.find((item) => item.id === indicatorId);
        if (!indicator)
            throw new SuccessError('INDICATOR_NOT_FOUND', indicatorId);
        if (!Number.isFinite(value))
            throw new SuccessError('INVALID_OBSERVATION', 'Progress observation must be numeric');
    ...
    progress(personId, indicatorId) {
        ...
        if (values.length >= 2) {
            if (indicator.direction === 'increase')
                direction = rate > 0 ? 'toward' : rate < 0 ? 'away' : 'neutral';
            if (indicator.direction === 'decrease')
                direction = rate < 0 ? 'toward' : rate > 0 ? 'away' : 'neutral';
            if (indicator.direction === 'maintain')
                direction = Math.abs(rate) <= 0.1 ? 'toward' : 'away';
        }
...
export function successAutomaton({ id = 'success', address = { mode: 'macro', gate: 14, line: 1, color: 1, tone: 1, base: 1 }, ledger = new SuccessLedger() } = {}) {
    return new Automaton({
        ...
        implementation: (input, { state }) => {
            if (input.type === 'define')
                return state.define(input.personId, input.purpose);
            if (input.type === 'observe')
                return state.observe(input.personId, input.indicatorId, input.value, input.context);
            if (input.type === 'propose')
                return state.propose(input.personId, input.indicatorId, input.proposal);
            if (input.type === 'summary')
                return state.summary(input.personId);
