...
export class GraphRuntime {
  ...
  async step(sessionId: string): Promise<RuntimeStepResult> {
    ...
    // Determine current dimensional stage (cycles 1-5)
    const dimensionalStage = (stepNumber % 5) + 1 as 1 | 2 | 3 | 4 | 5;

    // Build dimensional frame
    ...
    // Allocate capabilities and schedule tools
    await this.toolScheduler.schedule(state);

    // Update expression graph
  ...
  private areCompatible(a: ActiveNodeState, b: ActiveNodeState): boolean {
    ...
    // 2. Gate harmonic relationship (opposite or complementary)
    ...
    return (isHarmonic || isComplementary) && phaseAligned && 
