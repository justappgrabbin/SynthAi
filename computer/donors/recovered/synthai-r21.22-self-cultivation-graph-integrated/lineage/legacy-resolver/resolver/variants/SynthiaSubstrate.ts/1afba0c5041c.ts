...
export interface CoordinateAddress {
  side: 0 | 1;                   // 0 = Personality/Conscious, 1 = Design/Unconscious
  planet: number;                // 0-12 (13 planets)
  dimension: number;             // 0-4 (5 dimensions)
  gate: number;                  // 0-63 (64 gates)
  line: number;                  // 0-5 (6 lines)
  color: number;                 // 0-5 (6 colors)
  tone: number;                  // 0-5 (6 tones)
  base: number;                  // 0-4 (5 bases)
  degree: number;                // 0-59
  minute: number;                // 0-59
  second: number;                // 0-59
  arc: number;                   // 0-99
  zodiac: number;                // 0-11 (12 zodiac)
  season: number;                // 0-3 (4 seasons)
  houseZodiac: number;           // 0-11 (12 houses)
  houseSeason: number;            // 0-7 (8 houses)
}
...
export class CoordinateEncoder {
  ...
  static encode(coord: CoordinateAddress): [number, number] {
    const word1 = 
      (coord.side & 0x1) |
      ((coord.planet & 0xF) << 1) |
  ...
  static decode(word1: number, word2: number): CoordinateAddress {
    return {
      side: (word1 & 0x1) as 0 | 1,
      planet: (word1 >> 1) & 0xF,
  ...
  static toIndex(coord: CoordinateAddress): number {
  ...
  static sideOffset(side: 0 | 1): number {
    return side * NODES_PER_SIDE;
  }

  // Full linear address (0 to 138,239)
  static fullAddress(coord: CoordinateAddress): number {
...
export class SynthiaSubstrate {
  ...
  private centerGates: Map<string, number[]> = new Map([
    ['Head', [64, 61, 63]],
    ['Ajna', [47, 24, 4, 11]],
  ...
  activate(coord: CoordinateAddress, intensity: number = 1.0): void {
    ...
    if (intensity > 0) {
      ...
      this.activeAddresses[this.activeCount] = (packed[0] & 0xFFFF) | ((packed[1] & 0xFFFF) << 16);
      this.activeCount++;
    }
  }

  /** Get activation intensity at coordinate */
  getIntensity(coord: CoordinateAddress): number {
    return this.activation[CoordinateEncoder.fullAddress(coord)];
  }

  /** Get all active nodes (sparse) */
  getActiveNodes(): Array<{ coord: CoordinateAddress; intensity: number }> {
    const result: Array<{ coord: CoordinateAddress; intensity: number }> = [];
    for (let i = 0; i < this.activeCount; i++) {
      const packed = this.activeAddresses[i];
      const word1 = packed & 0xFFFF;
      const word2 = (packed >> 16) & 0xFFFF;
      // Note: This is a simplified decode — full decode needs both 32-bit words
      // For active node tracking, we store the full coordinate separately
    }
    return result;
  }
  ...
  computeEdges(coord: CoordinateAddress): number[] {
  ...
  computeCenters(coord: CoordinateAddress): string[] {
  ...
  resonance(a: CoordinateAddress, b: CoordinateAddress): number {
    let score = 0;
    let total = 0;
  ...
  nearestActive(target: CoordinateAddress): { coord: CoordinateAddress; distance: number } | null {
    let nearest: CoordinateAddress | null = null;
          ...
          for (let color = 0; color < CONSTANTS.COLORS; color++) {
            for (let tone = 0; tone < CONSTANTS.TONES; tone++) {
              for (let base = 0; base < CONSTANTS.BASES; base++) {
                const coord: CoordinateAddress = {
                  side: side as 0 | 1, planet: 0, dimension: 0,
  ...
  private computeDistance(a: CoordinateAddress, b: CoordinateAddress): number {
  ...
  transition(from: CoordinateAddress, to: CoordinateAddress): { 
    path: CoordinateAddress[]; 
    resonance: number;
    centers: string[];
  } {
    // Compute path through intermediate coordinates
    const path: CoordinateAddress[] = [from];
  ...
  whToDimension(wh: 'who' | 'what' | 'where' | 'when' | 'why'): number {
    switch (wh) {
  ...
  extractFromWH(input: { who?: string; what?: string; where?: string; when?: string; why?: string }): CoordinateAddress {
    // Default coordinate
    const coord: CoordinateAddress = {
  ...
  private extractGatesFromText(text: string): number[] {
  ...
  getActivationArray(): Float32Array {
