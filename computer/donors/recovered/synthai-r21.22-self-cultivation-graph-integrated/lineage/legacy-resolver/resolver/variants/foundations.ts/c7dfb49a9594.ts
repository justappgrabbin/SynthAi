...
export interface ToolMaterializationContribution {
  files?: ArtifactFile[];
...
export interface KleinToolAdapter {
  ...
  /** Optional export hook used only when a graph is materialized.
   * Runtime execution does not depend on this hook; adapters that can expose
   * source/manifests contribute them to the final artifact bundle.
   */
  materialize?(): Promise<ToolMaterializationContribution> | ToolMaterializationContribution;
}
