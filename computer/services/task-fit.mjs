...
export class TaskFitService {
  ...
  scoreTask({ task = {}, participants = [], sunGate = 16 } = {}) {
    ...
    const result = {
      task: { title: task.title ?? null, difficulty: task.difficulty ?? null },
      participantIds: team.map(p => p.id),
      fit,
      tier: tierFor(fit),
      eligible: eligibilityReasons.length === 0,
      eligibilityReasons,
      score: { axis: axisScore, ringBonus, gateBonus, sunBonus, raw },
      axisContributions,
  ...
  rankCandidates({ task = {}, candidates = [], sunGate = 16 } = {}) {
    const ranked = (candidates ?? []).map(candidate => {
      ...
      return { candidateId: String(candidate.id ?? candidate.name ?? 'participant'), ...result };
    }).sort((a, b) =>
      b.fit - a.fit
